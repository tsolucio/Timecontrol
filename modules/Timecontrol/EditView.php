...
if (!empty($_REQUEST['relatedto']) and getSalesEntityType($_REQUEST['relatedto'])=='HelpDesk') { // coming from TT, pickup data
	$rshd=$adb->pquery('select ticket_no,product_id from vtiger_troubletickets where ticketid=?',array($_REQUEST['relatedto']));
